...
```
python3 tools/extract_levels.py     # rebuild web/data from the catalog
python3 -m http.server 8000         # then open web/index.html
```
...
| `tools/arrangement.py` | Planar arrangement: splits the rope at crossings and extracts region polygons. |
| `web/` | The game. Vanilla JS and SVG, no build step, no dependencies. |
| `web/data/` | Generated. `index.json` plus one file per level. |

...
- **The current** runs inside it, as wide as the flow actually passing through
  ...
  rather than deduced.
- **Water going nowhere still moves**, at the same brightness and much the
  same speed. A holding cut takes the flow to zero everywhere, so without this
  the run that proves a *right* answer was the only one that froze — exactly
  backwards. Dimming and slowing it as well was three signals saying one thing,
  and it made a held cut look like a stalled game rather than like water
  hammering against it. Width alone carries throughput; the dry sink side and
  the readout already say whether anything is getting through.
